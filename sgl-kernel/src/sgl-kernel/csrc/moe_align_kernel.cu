...
SGLANG_FORCE_INLINE_DEVICE_FUNC void align_global_cumsum(int32_t* local_offsets /*src_and_dest*/, int32_t* local_offsets_buf, int* smem_ptr, const int tid,
...
SGLANG_FORCE_INLINE_DEVICE_FUNC void parallel_unaligned_local_cumsum(
  ...
  if (threadIdx.x < active_threads) {
    for (int i = threadIdx.x * kElementsPerThr + 1; i < MIN((threadIdx.x + 1) * kElementsPerThr + 1, num_experts + 1); ++i) {
